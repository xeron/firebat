class SowItemCommand < FireBatCommand
  ...
  def on_privmsg( cmd )
    text = cmd.args_tail(1,1)
	 if text =~ /^(\S) (.+)$/
	   keys = {"н" => "name","т" => "tip","л" => "lvl","п" => "flags_pr","о" => "flags_or","а" => "affects","в" => "weigth","к" => "slot","у" => "dmg", "з" => "spell", "с" => "skill", "м" => "modif","ц" => "color"}
	   field = keys[$1]
	   text = $2
    ...
    text = "%#{text}%"
	 ...
	 if n < 0
      ...
      if i = SowItem.find( :first, :conditions => ["#{field} like ?",text], :offset => n )
        keys1=["Требует для использования","Флаги предмета","Флаги оружия  ","Флаги запрета ","Аффекты       ","Вес","Вы можете одеть это на","Цвет","Оружейное умение","Повреждения","Заклинание","Умение","Модификаторы"]
        keys2=["lvl","flags_pr","flags_or","flags_zap","affects","weigth","slot","color","wp_skill","dmg","spell","skill","modif"]
        time = "Устарело!"
  ...
  def self.install    
    ...
    add_index :sow_items, :name, :name => "name", :unique => true
  end
